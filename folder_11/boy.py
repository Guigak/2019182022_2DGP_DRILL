...
class AUTO_RUN :
    ...
    def draw(self) :
        if self.dir == 1 :
            self.image.clip_composite_draw(self.frame * 100, 100, 100, 100, 0, '', self.x, self.y + 30, 200, 200)
        elif self.dir == -1:
            self.image.clip_composite_draw(self.frame * 100, 0, 100, 100, 0, '', self.x, self.y + 30, 200, 200)
        pass
...
class Boy:
    ...
    def handle_event(self, event) : # 소년이 스스로 이벤트를 처리할 수 있게
        # event는 키 이벤트이므로 이것을 내부RD 등으로 변환
