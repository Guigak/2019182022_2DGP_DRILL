...
class TileBackground:
    ...
    def draw(self):
        # fill here
        self.window_left = clamp(0, int(server.boy.x) - self.canvas_width // 2, self.w - self.canvas_height - 1)
        self.window_bottom = clamp(0, int(server.boy.y) - self.canvas_height // 2, self.h - self.canvas_height - 1)

        tile_left = (self.window_left // 800) % 3
        tile_right = ((self.window_left + self.canvas_width) // 800) % 3
        left_offset = self.window_left % 800

        tile_bottom = (self.window_bottom // 600) % 3
        tile_top = ((self.window_bottom + self.canvas_height) // 600) % 3
